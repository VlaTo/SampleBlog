...
namespace Microsoft.Extensions.DependencyInjection
{
    ...
    public static class IdentityServerBuilderConfigurationExtensions
    {
            ...
            IConfiguration? configuration)
        {
            ...
            builder.Services.TryAddEnumerable(
                ServiceDescriptor.Singleton<IConfigureOptions<ApiAuthorizationOptions>, ConfigureIdentityResources>(
                    services =>
                    {
                        var logger = services.GetRequiredService<ILogger<ConfigureIdentityResources>>();
                        var effectiveConfig = configuration ?? services.GetRequiredService<IConfiguration>().GetSection("IdentityServer:Identity");
            ...
            // and that prevents normal composition.
            builder.Services.AddSingleton<IEnumerable<IdentityResource>>(services =>
            {
                var options = services.GetRequiredService<IOptions<ApiAuthorizationOptions>>();
